...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    setLoading(true);
    const toastId = toast.loading("Forging new runesmith...");
    try {
      const userCredential = await createUserWithEmailAndPassword(auth, email, password);
      await updateProfile(userCredential.user, { displayName: name });
      toast.success("Welcome accepted to Valhalla", { id: toastId });
      navigate("/");
    } catch (error) {
      ...
      <div className="container">
        ...
        >
          <div className="auth-header text-center">
            ...
            <h1 className="auth-title">Forge Your <span className="text-violet">Soul</span></h1>
            <p className="auth-subtitle text-muted">Register to interact with the cosmic protocols</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="auth-form">
            ...
            <button type="submit" className="btn-aether w-full mt-8" disabled={loading}>
              {loading ? "Forging Identity..." : "Join the Realm"}
            </button>
          </form>
          ...
          <p className="auth-footer-text">
            Already forged? <Link to="/login" className="auth-link text-violet">Login Here</Link>
          </p>
        </motion.div>
      </div>
    </div>
